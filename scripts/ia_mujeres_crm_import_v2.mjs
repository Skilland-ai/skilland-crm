...
function companyWebsiteInput(url) {
  ...
  const candidate = String(url).split('|')[0]?.trim();
  if (!candidate || /^unknown$/i.test(candidate)) return undefined;
  if (!/^https?:\/\/[^\s]+$/i.test(candidate)) return undefined;

  return {
    primaryLinkLabel: 'Website',
    primaryLinkUrl: candidate,
    secondaryLinks: [],
  };
}
...
function contactKey(contact) {
  return [
    ...
    contact.role_title,
    contact.department_area,
    normalizeEmail(contact.email) || String(contact.email ?? '').trim(),
  ].join('|||');
}
...
class TwentyClient {
  ...
  async gqlData(query, variables = {}) {
    for (let attempt = 1; attempt <= 6; attempt += 1) {
      ...
      const rateLimited = parsed.errors.some(
...
function rankContact(contact) {
  ...
  const named =
    String(contact.contact_name ?? '').trim().toLowerCase() === 'unknown'
      ? 0
      : 20;
...
function buildOpportunityPlans(orgs, contactsByOrg) {
  ...
  for (const org of orgs) {
    ...
    if (org.organization_type === 'cabildo') {
      ...
      const knownAreas = areas.filter(
        (area) => normalizeString(area) !== 'unknown',
      );

      if (knownAreas.length > 1) {
...
function computeAudit(orgs, contacts, workspaceData, contactsByOrg) {
  ...
  for (const company of workspaceData.companies) {
    ...
    if (domain) companiesByDomain.set(domain, company);
  }
  ...
  for (const org of orgs) {
    ...
    if (domainMatch) {
      orgDomainMatches.push({
        ...
        domain,
        hasExistingOpportunities: Boolean(domainMatch.opportunities?.edges?.length),
      });
    }
  }
  ...
  return {
    ...
    plannedDealCount: buildOpportunityPlans(orgs, contactsByOrg).length,
    orgNameMatches,
    orgDomainMatches,
    personEmailMatches,
  };
...
function buildOpportunityData({
  ...
  return {
    ...
    island: org.island,
    municipality: org.municipality,
    sourceType: bestContact?.source_type || org.source_type,
    sourceUrl: bestContact?.source_url || org.source_url,
    sourceFile: bestContact?.source_file || org.source_file,
    icpSegment: bestContact?.icp_segment || org.icp_segment,
    qualityFlags: joinFlags(
      org.quality_flags,
...
async function ensureFields(client, objects, apply, report) {
  ...
  for (const [objectName, fields] of Object.entries(FIELD_DEFS)) {
    ...
    if (!object) throw new Error(`Object metadata not found for ${objectName}`);
...
async function ensureCustomTypesAvailable(client, apply, report) {
  ...
  for (let attempt = 1; attempt <= 10; attempt += 1) {
    ...
    for (const [typeName, fieldNames] of Object.entries(expected)) {
      ...
      if (missing.length) allReady = false;
    }
...
async function upsertPeople({
  ...
  for (const contact of contacts) {
    ...
    const existing = peopleByEmail.get(normalizeEmail(contact.email));
    ...
    if (!apply) {
      if (existing) {
        ...
        personIdsByKey.set(key, existing.id);
      } else {
        result.plannedCreates.push({
          sourceKey: key,
          email: contact.email,
    ...
    if (existing) {
      ...
      personIdsByKey.set(key, response.updatePerson.id);
      result.updated.push({
        sourceKey: key,
        email: contact.email,
    ...
    personIdsByKey.set(key, response.createPerson.id);
    result.created.push({
      sourceKey: key,
      email: contact.email,
...
async function upsertOpportunities({
  ...
  for (const plan of plans) {
    ...
    const contacts = plan.splitByArea
      ...
      : orgContacts;
    const bestContact = chooseBestContact(contacts);
    const personId = bestContact
      ? personIdsByKey.get(contactKey(bestContact))
      : null;
...
async function createOpportunityViews(client, objects, apply, report) {
  ...
  if (!object) throw new Error('Opportunity object metadata not found');
...
async function main() {
  ...
  const personIdsByKey = await upsertPeople({
    client,
    contacts,
    orgByName,
    companyIdsByOrg,
